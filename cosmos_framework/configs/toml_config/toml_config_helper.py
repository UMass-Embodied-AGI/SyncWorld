...
# ---------------------------------------------------------------------------
# The gripperhead FDM/IDM dataset node inside the PackingDataLoader tree. ``[gripperhead]`` fields
# land here unless listed individually below.
_GH: tuple[str, ...] = ("dataloader_train", "dataloader", "datasets", "gripperhead", "dataset")
# Where the flow-matching loss weights + distillation knobs live on the VFM model config.
_RF: tuple[str, ...] = ("model", "config", "rectified_flow_training_config")
...
PATH_REMAPS: dict[str, dict[tuple[str, ...], "tuple[str, ...] | None"]] = {
    ...
    "vfm": {
        ...
        # The text/VLM tokenizer is a DIFFERENT node from the video tokenizer (model.tokenizer).
        ("model", "text_tokenizer"): ("model", "config", "vlm_config", "tokenizer"),
        ...
        # --- [gripperhead] -> the FDM dataset node, with the few leaves that belong elsewhere
        # routed individually. Short TOML names map onto the dataset's longer kwarg names.
        ("gripperhead", "num_history"): _GH + ("num_history_frames",),
        ("gripperhead", "num_pred"): _GH + ("num_pred_frames",),
        ("gripperhead", "history_stride"): _GH + ("history_frame_stride",),
        ("gripperhead", "history_min"): _GH + ("history_min_frames",),
        ("gripperhead", "future_min"): _GH + ("future_min_frames",),
        ("gripperhead", "calib_segments"): _GH + ("calib_num_segments",),
        # Loss / distillation knobs are model-side, not dataset-side.
        ("gripperhead", "action_loss_weight"): _RF + ("action_loss_weight",),
        ("gripperhead", "gripper_loss_weight"): _RF + ("gripper_loss_weight",),
        ("gripperhead", "lambda_cons"): _RF + ("distill_lambda_cons",),
        ("gripperhead", "p_cons"): _RF + ("distill_p_cons",),
        # Derived in sft_config.gripperhead_derived_overrides, not emitted directly.
        ("gripperhead", "resume_action_heads"): None,
        ("gripperhead",): _GH,
        ("model",): ("model", "config"),
    },
    ...
    "vlm": {
        ...
        ("model", "text_tokenizer"): None,                                     # VFM-only routing
        ("gripperhead",): None,                                                # VFM-only recipe
