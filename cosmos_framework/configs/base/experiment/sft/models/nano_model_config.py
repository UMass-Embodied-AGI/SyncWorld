...
NANO_MODEL_CONFIG = dict(
    ...
    diffusion_expert_config=dict(
        ...
        # Rope grid max latent side (h & w). Default 20 = 640px/side. Higher-res
        # training (e.g. gripperhead 512_w2 mask = 1024px wide -> latent w=32) needs
        # this raised; set MAX_VAE_LATENT_SIDE in the env. Positions <=19 are
        # unchanged, so the pretrained low-res behavior is preserved.
        max_vae_latent_side_after_patchify=int(os.environ.get("MAX_VAE_LATENT_SIDE", "20")),
        patch_spatial=2,
