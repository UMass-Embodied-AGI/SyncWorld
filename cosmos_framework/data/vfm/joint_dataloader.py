        ...
        while True:
            ...
            while True:
                ...
                try:
                    ...
                    # ``JointDataLoader`` creates the inner iterator once during
                    # construction so its pre-warmed sample is not duplicated.
                    # A finite map-style source eventually exhausts that iterator;
                    # rebuild it here so the next packed batch starts a new epoch.
                    # Without this reset, the trainer repeatedly creates an empty
                    # outer iterator and spins forever at the first epoch boundary.
                    self.dataloaders[0] = iter(self.dataloader_list[0])
                    if len(output_batch) > 0:
                        break
                    try:
                        output = self._get_next_sample(0)
