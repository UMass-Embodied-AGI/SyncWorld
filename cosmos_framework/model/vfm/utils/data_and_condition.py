...
class GenerationDataClean:
    ...
    # Multi-item action (calibration/history segments): number of action items per sample.
    # When set, x0_tokens_action is a FLAT list of per-item action tensors (one block per vision
    # item, e.g. [calib0, calib1, ..., history, current+future]) mirroring num_vision_items_per_sample.
    # Each block is aligned in the packer to its vision item's mRoPE temporal origin. None for the
    # standard single-block case (one action tensor per sample) -> backward-compatible.
