...
class RectifiedFlowTrainingConfig:
    ...
    # Extra multiplier on the LAST action channel (the gripper) in the action flow-matching loss.
    # 1.0 (default) => no-op / all recipes unchanged; e.g. 2.0 up-weights gripper accuracy for a policy.
    ...
    # Paired teacher/student distillation (robotics-wan p_cons parity, adapted). On a distill
    # step the trainer rewrites the batch into consecutive teacher/student pairs (2i, 2i+1) built
    # from the SAME encoded sample: teacher keeps full conditioning, student has its CALIBRATION
    # items nulled. eps + timestep are shared within the pair, and a stop-grad consistency loss
    # (student vs detached teacher) is added over the TEACHER's generation region (future video for
    # forward_dynamics, action for inverse_dynamics — auto-selected by 1-condition_mask). Requires
    # calibration items in the batch and an even per-rank batch size.
    #   distill_lambda_cons: weight of the consistency term (0.0 => feature OFF, byte-identical).
    #   distill_p_cons: probability a given step forms pairs (only used when distill_lambda_cons>0).
    distill_lambda_cons: float = 0.0
    distill_p_cons: float = 0.0
