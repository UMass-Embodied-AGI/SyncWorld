...
# --- Python 3.13 -> 3.12 checkpoint-unpickle compat shim -----------------------------------------
# Base checkpoints (e.g. Cosmos3-Nano) are saved under Python 3.13, whose pathlib is a package and
# pickles PosixPath as ``pathlib._local.PosixPath``. Loading them under a Python 3.12 conda (the
# GB300/aarch64 env) fails: torch DCP's read_metadata() -> pickle.load() raises "No module named
# 'pathlib._local'", which DCP swallows into a warning and surfaces later as "metadata is None".
# Register a shim module aliasing pathlib's classes so the unpickle resolves. No-op on Python >= 3.13
# (the real module imports cleanly), so grandteton is unaffected.
try:  # pragma: no cover - environment dependent
    ...
    _pathlib_local = _types.ModuleType("pathlib._local")
    for _name in dir(_pathlib):
        if not _name.startswith("__"):
            setattr(_pathlib_local, _name, getattr(_pathlib, _name))
    _sys.modules["pathlib._local"] = _pathlib_local
...
# GB300/aarch64: cuDNN 9.17 on Grace/sm_103 cannot bring up a working conv engine — its AUTO mode
# resolves to the JIT (runtime-compiled) sublib, whose dlopen of libnvrtc fails
# (CUDNN_STATUS_SUBLIBRARY_LOADING_FAILED at the first conv), and forcing CUDNN_LIB_CONFIG=FULL
# (precompiled engines) instead HANGS at cuDNN init (no precompiled sm_103 kernels in this build). So we
# disable cuDNN on aarch64 and let conv ops use torch's native kernels (numerically fine; VAE convs are
# a small slice of the step). This is the verified-working GB300 config. Set COSMOS_DISABLE_CUDNN=0 to
# re-enable cuDNN once a cuDNN build with sm_103 precompiled engines is available. No-op on x86.
if platform.machine() == "aarch64" and os.environ.get("COSMOS_DISABLE_CUDNN", "1") == "1":
