...
class OmniMoTModel(ImaginaireModel):
    ...
    # ------------------------------------------------------------------ paired distillation (Feature B)
    def _maybe_apply_distill_pairing(self, data_batch: dict, iteration: int) -> bool:
        """Feature B (paired teacher/student distillation, robotics-wan p_cons parity, adapted).

        On a distill step, rewrite the batch IN-PLACE into consecutive teacher/student pairs
        ``(2i, 2i+1)`` built from the SAME sample: the STUDENT (odd slot) is a copy of the TEACHER
        (even slot) with its CALIBRATION items nulled (black video -> VAE(black), zero action). The
        caller then shares eps + timestep within each pair and adds a stop-grad consistency loss on the
        teacher's generation region (:meth:`_compute_distill_consistency_loss`).

        Returns True iff pairing was applied. No-op (byte-identical, returns False) unless
        ``distill_lambda_cons>0`` AND a per-iteration coin < ``distill_p_cons`` AND calibration is
        present AND the per-rank batch is even. The coin is seeded by ``iteration`` so all CP/DP ranks
        make the SAME decision (CP ranks share one broadcast batch and must stay consistent).
        """
        rf_cfg = self.config.rectified_flow_training_config
        lam = float(getattr(rf_cfg, "distill_lambda_cons", 0.0) or 0.0)
        p_cons = float(getattr(rf_cfg, "distill_p_cons", 0.0) or 0.0)
        if lam <= 0.0 or p_cons <= 0.0:
            return False
        video = data_batch.get(self.input_video_key)
        if not isinstance(video, list):
            return False  # single stacked-tensor batch (no multi-item calibration) -> not applicable
        batch_size = len(video)
        if batch_size < 2 or batch_size % 2 != 0:
            return False  # need an even per-rank batch to form (2i, 2i+1) pairs
        num_calib = data_batch.get("num_calib_items")
        if num_calib is None:
            return False  # dataset didn't report calibration item counts -> can't null the student calib
        _nc = num_calib if torch.is_tensor(num_calib) else torch.as_tensor(num_calib)
        if int(_nc.max().item()) <= 0:
            return False  # no calibration items to null -> student would equal teacher (pointless)

        # Rank-consistent per-iteration coin (deterministic given the iteration).
        coin_gen = torch.Generator()
        coin_gen.manual_seed((int(iteration) * 2654435761) % (2**31))
        if float(torch.rand((), generator=coin_gen).item()) >= p_cons:
            return False
        if getattr(rf_cfg, "independent_action_schedule", False):
            raise NotImplementedError(
                "Paired distillation (distill_lambda_cons>0) currently assumes "
                "independent_action_schedule=False (action shares the vision sigma)."
            )
        ...
        for i in range(batch_size // 2):
            t, s = 2 * i, 2 * i + 1
            # Copy every OTHER per-sample entry teacher(t) -> student(s). Video/action are handled
            # below (they need calibration nulling). Shared references are safe: encode() only reads.
            for key, val in data_batch.items():
                if key in (self.input_video_key, "action"):
                    continue
                if isinstance(val, list) and len(val) == batch_size:
                    val[s] = copy.copy(val[t]) if key == "sequence_plan" else val[t]
                elif torch.is_tensor(val) and val.dim() >= 1 and val.shape[0] == batch_size:
                    val[s] = val[t].clone()
            k_calib = _as_int(num_calib[t])
            # Student video: null the K calibration items (black -> in-distribution VAE(black), matching
            # the dataset's own calibration-null placeholder); share the remaining items (read-only).
            video[s] = [torch.zeros_like(it) if k < k_calib else it for k, it in enumerate(video[t])]
            if isinstance(action, list) and isinstance(action[t], list):
                action[s] = [torch.zeros_like(a) if k < k_calib else a for k, a in enumerate(action[t])]
        return True

    @staticmethod
    def _pair_item_offsets(num_items_per_sample: list[int] | None, n_flat: int) -> list[int]:
        """Prefix-sum offsets so sample ``i`` owns flat items ``[off[i] : off[i]+n[i]]``. When
        ``num_items_per_sample`` is None (one item per sample) offsets are ``[0,1,2,...]``."""
        if num_items_per_sample is None:
            return list(range(n_flat + 1))
        offs = [0]
        for n in num_items_per_sample:
            offs.append(offs[-1] + int(n))
        return offs

    def _share_eps_within_pairs(self, eps_list: list, num_items_per_sample: list[int] | None) -> None:
        """Copy each teacher (even) sample's per-item eps into the following student (odd) sample so a
        pair denoises the SAME noise realization (only the nulled calibration differs). In-place."""
        offs = self._pair_item_offsets(num_items_per_sample, len(eps_list))
        n_samples = len(num_items_per_sample) if num_items_per_sample is not None else len(eps_list)
        for i in range(0, n_samples - 1, 2):
            t_off, s_off = offs[i], offs[i + 1]
            t_n = offs[i + 1] - offs[i]
            s_n = offs[i + 2] - offs[i + 1]
            for k in range(min(t_n, s_n)):
                eps_list[s_off + k] = eps_list[t_off + k].clone()

    def _compute_distill_consistency_loss(
        self,
        *,
        out_net: dict,
        data_batch_packed: "PackedSequence",
        timesteps: torch.Tensor,
        ts_action: torch.Tensor,
        is_image_batch: bool,
        distill_info: dict,
    ) -> tuple[torch.Tensor, dict]:
        """Feature B consistency loss: for each teacher/student pair, MSE(student_pred,
        stopgrad(teacher_pred)) over the TEACHER's generation region (``1-condition_mask``), time-weighted
        and active-normalized exactly like the flow-matching loss. Because forward makes ACTION
        all-conditioning and inverse makes VISION all-conditioning, the teacher mask auto-selects the right
        target modality (future video for forward, action for inverse) with no mode branch."""
        lam = float(distill_info["lambda_cons"])
        total = 0.0
        logs: dict[str, torch.Tensor] = {}

        def _gather(preds, cmask, num_items):
            offs = self._pair_item_offsets(num_items, len(preds))
            n_samples = len(num_items) if num_items is not None else len(preds)
            s_pred, t_pred, masks, rows = [], [], [], []
            for i in range(0, n_samples - 1, 2):
                t_n = offs[i + 1] - offs[i]
                s_n = offs[i + 2] - offs[i + 1]
                for k in range(min(t_n, s_n)):
                    ti, si = offs[i] + k, offs[i + 1] + k
                    s_pred.append(preds[si])
                    t_pred.append(preds[ti].detach())
                    masks.append(cmask[ti])
                    rows.append(si)
            return s_pred, t_pred, masks, rows

        # Vision consistency (active only when the teacher has noised vision tokens => forward / masked history).
        nvis = distill_info.get("num_vision_items_per_sample")
        if self.config.vision_gen and data_batch_packed.vision is not None:
            preds_v = out_net["preds_vision"]
            cmask_v = data_batch_packed.vision.condition_mask
            s_pred, t_pred, masks, rows = _gather(preds_v, cmask_v, nvis)
            if s_pred:
                has_valid = any(float((1.0 - m).sum().item()) > 0 for m in masks)
                ts = timesteps[torch.tensor(rows, device=timesteps.device)] if timesteps.dim() > 1 else timesteps
                rf = self.rectified_flow_image if is_image_batch else self.rectified_flow_video
                cons_v, _ = self._compute_flow_matching_loss(
                    pred=s_pred, target=t_pred, condition_mask=masks, timesteps=ts,
                    has_valid_tokens=has_valid, rectified_flow=rf, normalize_by_active=True,
                )
                total = total + cons_v * lam
                logs["distill_consistency_loss_vision"] = cons_v.detach()

        # Action consistency (active only when the teacher has noised action tokens => inverse).
        nact = distill_info.get("num_action_items_per_sample")
        if self.config.action_gen and data_batch_packed.action is not None:
            preds_a = out_net["preds_action"]
            cmask_a = data_batch_packed.action.condition_mask
            rad_a = data_batch_packed.action.raw_action_dim
            offs = self._pair_item_offsets(nact, len(preds_a))
            n_samples = len(nact) if nact is not None else len(preds_a)
            s_pred, t_pred, masks, rows, rad = [], [], [], [], []
            for i in range(0, n_samples - 1, 2):
                t_n = offs[i + 1] - offs[i]
                s_n = offs[i + 2] - offs[i + 1]
                for k in range(min(t_n, s_n)):
                    ti, si = offs[i] + k, offs[i + 1] + k
                    s_pred.append(preds_a[si])
                    t_pred.append(preds_a[ti].detach())
                    masks.append(cmask_a[ti])
                    rows.append(si)
                    rad.append(rad_a[ti] if rad_a is not None else None)
            if s_pred:
                has_valid = any(float((1.0 - m).sum().item()) > 0 for m in masks)
                ts = ts_action[torch.tensor(rows, device=ts_action.device)] if ts_action.dim() > 1 else ts_action
                cons_a, _ = self._compute_flow_matching_loss(
                    pred=s_pred, target=t_pred, condition_mask=masks, timesteps=ts,
                    has_valid_tokens=has_valid, rectified_flow=self.rectified_flow_action,
                    raw_action_dim=(rad if rad_a is not None else None), normalize_by_active=True,
                )
                total = total + cons_a * lam
                logs["distill_consistency_loss_action"] = cons_a.detach()
    ...
    def training_step(
        ...
        # Feature B (paired distillation): on a distill step, rewrite the batch into teacher/student
        # pairs (student = calibration-nulled copy of teacher) BEFORE tokenization/encoding so the whole
        # downstream path (text, plans, encode, counts) sees the paired batch. No-op / byte-identical
        # unless distill_lambda_cons>0 fires. When active, eps + timestep are shared per pair (below) and
        # a stop-grad consistency loss is added in _compute_losses.
        distill_active = self._maybe_apply_distill_pairing(data_batch, iteration)
        ...
        # Feature B: share the timestep/sigma within each teacher/student pair (even -> odd) so both
        # denoise at the SAME noise level (only the nulled calibration differs). Rows are per-sample and
        # pairs are consecutive; action reuses the vision sigma (independent_action_schedule stays off).
        if distill_active:
            timesteps_vision[1::2] = timesteps_vision[0::2]
            sigmas_vision[1::2] = sigmas_vision[0::2]
        ...
        gen_data_noised = self._add_noise_to_input(
            ...
            iteration=iteration,
            share_within_pairs=distill_active,  # Feature B: share eps within teacher/student pairs
        )
        ...
        # Feature B: stop-grad consistency loss between each teacher/student pair over the teacher's
        # generation region (auto-selects future video for forward, action for inverse). No-op otherwise.
        if distill_active:
            ts_action = timesteps_vision if timesteps_action is None else timesteps_action
            cons_loss, cons_logs = self._compute_distill_consistency_loss(
                out_net=out_net,
                data_batch_packed=packed_sequence,
                timesteps=timesteps_vision,
                ts_action=ts_action,
                is_image_batch=gen_data_clean.is_image_batch,
                distill_info={
                    "lambda_cons": float(self.config.rectified_flow_training_config.distill_lambda_cons),
                    "num_vision_items_per_sample": gen_data_clean.num_vision_items_per_sample,
                    "num_action_items_per_sample": gen_data_clean.num_action_items_per_sample,
                },
            )
            loss = loss + cons_loss
            losses_dict.update(cons_logs)
    ...
    def _compute_flow_matching_loss(
        ...
        return compute_flow_matching_loss(
            ...
            last_channel_weight=last_channel_weight,
        )
        ...
        if self.config.action_gen:
            if data_batch_packed.action is not None:
                ...
                fm_loss_action, _ = self._compute_flow_matching_loss(
                    ...
                    normalize_by_active=normalize_by_active,
                    # Up-weight the gripper (last action channel) in the loss, e.g. gripper_loss_weight=2.0.
                    # Default 1.0 => no-op (all other recipes unchanged).
                    last_channel_weight=getattr(rf_cfg, "gripper_loss_weight", 1.0),
                )

                # action_loss_weight scales the action loss's contribution to total_loss. Some recipes
                # scale UP a tiny action loss (e.g. DROID, weight 10) to match a larger video loss; the
                # gripperhead inverse/joint recipe instead scales DOWN its LARGE action loss (default
                # 0.05 via [gripperhead].action_loss_weight) so it doesn't dominate the small video loss in
                # joint training. Log BOTH the raw action loss (the coefficient-independent action-precision
                # curve) and its weighted contribution to total_loss so wandb tracks each separately.
                total_loss += fm_loss_action * rf_cfg.action_loss_weight
                losses_dict["flow_matching_loss_action"] = fm_loss_action
                losses_dict["flow_matching_loss_action_weighted"] = (
                    fm_loss_action * rf_cfg.action_loss_weight
                ).detach()

                # --- Per-mode loss split (joint fdm/idm/policy training) -------------------------
                # In 3-way joint mode each sample GENERATES a different modality (its condition_mask
                # says which): fdm -> video only, idm -> action only, policy -> both. The aggregate
                # vision/action loss mixes them; here we log each mode's loss by re-running the SAME
                # loss on that mode's sample subset -- identical (active-normalized, time-weighted)
                # aggregation as the modality loss. The POLICY-mode subset is logged under the BASE key
                # (flow_matching_loss_vision / _action) -- the SAME key a pure-policy run uses (whose
                # aggregate is itself policy-only) -- so wandb overlays the two runs on ONE chart. The
                # fdm/idm modes get suffixed keys (_fdm / _idm).
                # Logging-only: no_grad + detached scalars added to losses_dict -> output_batch ->
                # WandbCallback (+ metrics.jsonl); never touches total_loss/backward. Only emitted when
                # the batch mixes modes (pure runs stay byte-identical); wrapped so it can't break training.
                try:
                    v_cm = (
                    ...
                    a_cm = data_batch_packed.action.condition_mask
                    if v_cm is not None and a_cm is not None and len(v_cm) == len(a_cm):
                        B = len(v_cm)
                        vg = [bool((1.0 - v_cm[i]).sum() > 0) for i in range(B)]  # sample generates video
                        ag = [bool((1.0 - a_cm[i]).sum() > 0) for i in range(B)]  # sample generates action
                        if len({(vg[i], ag[i]) for i in range(B)}) > 1:  # modes mixed -> joint
                            fdm = [i for i in range(B) if vg[i] and not ag[i]]
                            idm = [i for i in range(B) if ag[i] and not vg[i]]
                            pol = [i for i in range(B) if vg[i] and ag[i]]
                            rf_v = self.rectified_flow_image if is_image_batch else self.rectified_flow_video
                            with torch.no_grad():
                                for name, idx in (("fdm", fdm), ("policy", pol)):
                                    if idx:
                                        lv, _ = self._compute_flow_matching_loss(
                                            pred=[out_net["preds_vision"][i] for i in idx],
                                            target=[gen_data_noised.vt_target_vision[i] for i in idx],
                                            condition_mask=[v_cm[i] for i in idx],
                                            timesteps=timesteps[idx],
                                            has_valid_tokens=True,
                                            rectified_flow=rf_v,
                                            normalize_by_active=normalize_by_active,
                                        )
                                        # policy-mode -> the BASE key (same key a pure-policy run logs,
                                        # since its aggregate IS policy-only) so wandb overlays both runs
                                        # on ONE chart; fdm -> suffixed. Overwrites the joint 3-way
                                        # aggregate in the LOG only (total_loss still uses fm_loss_vision).
                                        vkey = "flow_matching_loss_vision" if name == "policy" else f"flow_matching_loss_vision_{name}"
                                        losses_dict[vkey] = lv.detach()
                                if ts_action.shape[0] == B:  # action timesteps are per-sample -> indexable
                                    rad = data_batch_packed.action.raw_action_dim
                                    for name, idx in (("idm", idm), ("policy", pol)):
                                        if idx:
                                            la, _ = self._compute_flow_matching_loss(
                                                pred=[out_net["preds_action"][i] for i in idx],
                                                target=[gen_data_noised.vt_target_action[i] for i in idx],
                                                condition_mask=[a_cm[i] for i in idx],
                                                timesteps=ts_action[idx],
                                                has_valid_tokens=True,
                                                rectified_flow=self.rectified_flow_action,
                                                raw_action_dim=([rad[i] for i in idx] if rad is not None else None),
                                                normalize_by_active=normalize_by_active,
                                                last_channel_weight=getattr(rf_cfg, "gripper_loss_weight", 1.0),
                                            )
                                            akey = "flow_matching_loss_action" if name == "policy" else f"flow_matching_loss_action_{name}"
                                            losses_dict[akey] = la.detach()
                except Exception:  # per-mode logging split must never break training
                    pass
            else:
    ...
    def _add_noise_to_input(
        ...
        # Feature B: share eps within teacher/student pairs so a pair denoises the SAME noise realization.
        if share_within_pairs:
            self._share_eps_within_pairs(epsilon_vision, gen_data_clean.num_vision_items_per_sample)
        ...
        if self.config.action_gen and x0_action is not None and len(x0_action) > 0:
            ...
            if all_actions_are_conditioning:
                ...
                # Feature B: share action eps within teacher/student pairs (same as vision).
                if share_within_pairs:
                    self._share_eps_within_pairs(epsilon_action, gen_data_clean.num_action_items_per_sample)
    ...
    def _prepare_inference_data(
        ...
        for sample_idx in range(n_sample):
            ...
            # Action noise is built per flat action block (x0_tokens_action) — extend one seed per
            # action item, mirroring vision, so multi-action-item samples ([hist_act, cf_act]) index safely.
            num_act_items = (
                gen_data_clean.num_action_items_per_sample[sample_idx]
                if gen_data_clean.num_action_items_per_sample is not None
                else 1
            )
            seed_dict["action"].extend([seed[sample_idx]] * num_act_items)
        ...
        for i in range(n_sample):
            ...
            if noise_action_list is not None and sequence_plans[i].has_action:
                # Multi-action-item (e.g. history-as-item FDM: [hist_act, cf_act]) — append EACH
                # action block for this sample, mirroring the vision-item loop above. Falls back to a
                # single block when num_action_items_per_sample is None (standard single-action FDM).
                n_act = (
                    gen_data_clean.num_action_items_per_sample[i]
                    if gen_data_clean.num_action_items_per_sample is not None
                    else 1
                )
    ...
    def _get_velocity(
        ...
        *,
        ...
        null_action=True zeroes the action tokens the network embeds (the conditioning action baked
        into noise_x) -> the unconditional branch for action classifier-free guidance. Needed because
        the action values come from noise_x here, NOT from gen_data_clean.x0_tokens_action.
        ...
        num_action_items = gen_data_clean.num_action_items_per_sample  # None for single-action-item
        has_sound = self.config.sound_gen and any(plan.has_sound for plan in sequence_plans)
        ...
        for i in range(n_samples):
            ...
            if has_action and noise_x_action is not None:
                ...
                # Multi-action-item: split out EACH action block for this sample (mirrors vision loop).
                n_act = num_action_items[i] if num_action_items is not None else 1
                for _ in range(n_act):
                    action_shape = gen_data_clean.x0_tokens_action[idx_action].shape
                    action_dim = int(torch.prod(torch.tensor(action_shape)))
                    act_tok = noise_x[i][offset : offset + action_dim].reshape(action_shape)  # [T,action_dim]
                    if null_action:
                        # action-CFG unconditional branch. Null sentinel = 6D delta zeroed + gripper
                        # (last REAL dim) = -1, matching the dataloader's _null_action: an all-zero 7D
                        # euler action is a VALID stay-still action (ambiguous), so gripper=-1 (OOD, gripper
                        # is binarized to {0,1}) marks the null unambiguously.
                        act_tok = torch.zeros_like(act_tok)
                        _rad = (
                            gen_data_clean.raw_action_dim[i]
                            if gen_data_clean.raw_action_dim is not None and gen_data_clean.raw_action_dim[i] is not None
                            else None
                        )
                        if _rad is not None and int(_rad) >= 1:
                            act_tok[..., int(_rad) - 1] = -1.0
                    noise_x_action.append(act_tok)
                    offset += action_dim
                    idx_action += 1
        ...
        gen_data_for_packing = GenerationDataClean(
            ...
            num_action_items_per_sample=num_action_items,
        )
        ...
        for i in range(n_samples):
            ...
            if velocity_action is not None and sequence_plans[i].has_action:
                # Multi-action-item: emit velocity for EACH action block (matches the noise concat
                # order in _prepare_inference_data and the split above).
                n_act = num_action_items[i] if num_action_items is not None else 1
                for _ in range(n_act):
    ...
    def _build_null_action_inference_state(self, data_batch: Dict, seed, has_negative_prompt: bool):
        """Re-prepare inference data with the action set to the NULL sentinel (for action classifier-free
        guidance). Matches the dataloader's _null_action: 6D delta zeroed + gripper (last dim) = -1.
        An all-zero 7D euler action is a VALID stay-still action (ambiguous), so gripper=-1 (OOD, gripper
        is binarized to {0,1}) is the unambiguous unconditional branch. Returns
        ``(sequence_plans, gen_data_clean)`` re-encoded with the null action; the real branch's noise/text
        are reused. NOTE: the operative action null at sampling time is ``_get_velocity(null_action=True)``."""
        def _null_like(a: torch.Tensor) -> torch.Tensor:
            o = torch.zeros_like(a)
            if o.shape[-1] > 0:
                o[..., -1] = -1.0  # gripper (last raw dim) sentinel
            return o
        null_batch = dict(data_batch)
        action = null_batch.get("action")
        if isinstance(action, (list, tuple)):
            null_batch["action"] = [_null_like(a) for a in action]
        elif torch.is_tensor(action):
            null_batch["action"] = _null_like(action)
        sp_null, gdc_null, _ct, _uct, _noise = self._prepare_inference_data(null_batch, seed, has_negative_prompt)
        return sp_null, gdc_null
    ...
    def generate_samples_from_batch(
        ...
        **kwargs,
        ...
        # Action classifier-free guidance: when action_guidance != 1.0, re-prepare
        # an inference state with the action ZEROED (matches the dataloader's
        # null-action) and blend velocities below. Default 1.0 -> no-op (existing
        # inference is byte-identical).
        null_action_state = None
        if action_guidance != 1.0:
            null_action_state = self._build_null_action_inference_state(data_batch, seed, has_negative_prompt)
        ...
        # Action-CFG: wrap the (real-action) velocity_fn so each step also runs a
        # null-action forward and blends: v = v_null + action_guidance*(v_real - v_null).
        # Gated on action_guidance != 1.0, so the default path above is untouched.
        # FSDP-safe: action_guidance is a scalar shared by all ranks, so every rank
        # runs the identical +1 forward per step.
        if null_action_state is not None:
            _velocity_fn_real = velocity_fn
            _sp_null, _gdc_null = null_action_state

            def velocity_fn(noise_x: list[torch.Tensor], timestep: torch.Tensor) -> list[torch.Tensor]:  # noqa: F811
                v_real = _velocity_fn_real(noise_x, timestep)
                ts = timestep.repeat(len(noise_x), 1)
                v_null = self._get_velocity(
                    net=net,
                    noise_x=noise_x,
                    timestep=ts,
                    text_tokens=cond_tokens,
                    sequence_plans=_sp_null,
                    gen_data_clean=_gdc_null,
                    skip_text_tokens=False,
                    null_action=True,  # zero the action tokens in noise_x (the real fix; _gdc_null alone is ignored)
                )
                return [vn + action_guidance * (vr - vn) for vr, vn in zip(v_real, v_null)]
        ...
        num_action_items = getattr(gen_data_clean, "num_action_items_per_sample", None)
        ...
        for i in range(n_sample):
            ...
            # Extract action if present. Multi-item (calibration/history + current+future): the action
            # blocks are packed after the vision blocks in item order; loop past them all like vision and
            # return ONLY the last (the current+future block — the only one generated in inverse/policy;
            # the calib/history blocks are conditioning). Single-item: n_action=1, unchanged. Without the
            # loop, offset landed on the FIRST (history/calib) block, so multi-item inverse returned the
            # wrong (conditioning) action tensor instead of the predicted current+future one.
            if self.config.action_gen and sequence_plans[i].has_action:
                assert gen_data_clean.x0_tokens_action is not None
                n_action = num_action_items[i] if num_action_items is not None else 1
                for j in range(n_action):
                    action_shape = gen_data_clean.x0_tokens_action[idx_action + j].shape
                    action_dim = int(torch.prod(torch.tensor(action_shape)))
                    if j == n_action - 1:  # the last action item is the generation target
    ...
    def get_data_and_condition(self, data_batch: dict[str, torch.Tensor], iteration: int = 1) -> GenerationDataClean:
        ...
        if "num_vision_items_per_sample" not in data_batch:
            ...
            if has_multiple_vision_per_sample:
                ...
                # Flatten [[items...] per sample] -> flat list of items. For VIDEO each item must be
                # (C,T,H,W): _normalize_video_databatch_inplace re-adds the batch dim via
                # torch.stack([item]), so strip any leading (1,) the joint dataloader added. Images
                # keep the legacy unsqueeze (their _augment_image_dim path expects it).
                if is_image_batch:
                    data_batch[media_key] = [item.unsqueeze(0) for sublist in sample_vision_list for item in sublist]
                else:
                    data_batch[media_key] = [
                        (item.squeeze(0) if (torch.is_tensor(item) and item.dim() == 5 and item.shape[0] == 1) else item)
        ...
        # --- Multi-item action (calibration/history blocks), mirroring the vision multi-item path.
        # When a sample's "action" is a list of >1 tensors, flatten to a per-item list + record counts
        # (so the packer aligns each action block to its vision item), flatten the per-sample
        # raw_action_dim list, and expand domain_id to per-action-item.
        if "num_action_items_per_sample" not in data_batch:
            sample_action_list = data_batch.get("action", None)
            has_multiple_action_per_sample = isinstance(sample_action_list, list) and any(
                isinstance(a, (list, tuple)) and len(a) > 1 for a in sample_action_list
            )
            num_action_items_per_sample: list[int] | None = (
                [len(a) for a in sample_action_list] if has_multiple_action_per_sample else None
            )
            data_batch["num_action_items_per_sample"] = num_action_items_per_sample
            if has_multiple_action_per_sample:
                # Action items may carry a leading batch dim from the joint dataloader ((1,T,D));
                # squeeze it so each block is (T,D) as the packer expects.
                def _sq_act(a):
                    return a.squeeze(0) if (torch.is_tensor(a) and a.dim() == 3 and a.shape[0] == 1) else a
                data_batch["action"] = [
                    _sq_act(item)
                    for sub in sample_action_list
                    for item in (sub if isinstance(sub, (list, tuple)) else [sub])
                ]
                rad = data_batch.get("raw_action_dim", None)
                if isinstance(rad, list) and rad and isinstance(rad[0], (list, tuple)):
                    data_batch["raw_action_dim"] = [r for sub in rad for r in sub]
                dom = data_batch.get("domain_id", None)
                if isinstance(dom, list):
                    data_batch["domain_id"] = [
                        dom[i] for i, n in enumerate(num_action_items_per_sample) for _ in range(n)
                    ]
        else:
            num_action_items_per_sample = data_batch["num_action_items_per_sample"]

        # image_size: for multi-item vision, flatten the per-sample [[s..],[s..]] list so it aligns
        # 1:1 with the flattened x0_tokens_vision (both length = total vision items).
        if num_vision_items_per_sample is not None:
            img_sz = data_batch.get("image_size", None)
            if isinstance(img_sz, list) and img_sz and isinstance(img_sz[0], (list, tuple)):
                data_batch["image_size"] = [s for sub in img_sz for s in sub]
        ...
        # For multi-item action (calibration), the packer indexes fps_action per-action-item, so
        # expand the per-sample fps to one entry per action item.
        if num_action_items_per_sample is not None and fps_action is not None:
            fps_action = torch.stack(
                [fps_action[i] for i, n in enumerate(num_action_items_per_sample) for _ in range(n)]
            )
        ...
        return GenerationDataClean(
            ...
            num_action_items_per_sample=num_action_items_per_sample,
            raw_action_dim=raw_action_dim,
        )
