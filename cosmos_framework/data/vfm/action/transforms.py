...
def augment_action_caption_for_inference(
    caption: str,
    *,
    viewpoint: str,
    num_frames: int,
    ...
    """Apply the same string-caption metadata augmentors used during training.

    Policy servers do not run the dataset transform pipeline, but their text tokens
    must still match it. A zero-element video carries the required shape without
    allocating an image tensor; the existing augmentors only inspect its shape.
    """
    data_dict: dict = {
        "ai_caption": caption,
        "viewpoint": viewpoint,
        "video": torch.empty((0, int(num_frames), 0, 0)),
        "conditioning_fps": float(fps),
        "image_size": (int(height), int(width)),
    }
    augmentors = []
    if append_viewpoint_info:
        augmentors.append(
            ViewpointTextInfo(
                input_keys=["ai_caption", "viewpoint"],
                output_keys=["ai_caption"],
                args={"caption_key": "ai_caption", "viewpoint_key": "viewpoint", "enabled": True},
            )
        )
    if append_duration_fps_timestamps:
        augmentors.append(
            DurationFPSTextTimeStamps(
    ...
    if append_resolution_info:
        augmentors.append(
            ResolutionTextInfo(
                ...
                output_keys=["ai_caption"],
                args={"caption_key": "ai_caption", "video_key": "video", "enabled": True},
            )
        )
...
def build_sequence_plan_from_mode(
    ...
    if mode == "forward_dynamics":
        # Condition on the first ``num_condition_latent_frames`` latent frames
        # (past video / history). Default 1 == legacy single-anchor behavior.
        condition_frame_indexes_vision = list(range(max(1, num_condition_latent_frames)))
    elif mode in ["image2video", "policy"]:
...
class ActionTransformPipeline:
    ...
    def __init__(
        ...
        # Number of past (history) pixel frames used as clean conditioning for
        # forward_dynamics; converted to latent-frame count when building the plan.
    ...
    def __call__(self, data_dict: dict, resolution: str | None) -> dict:
        ...
        # Multi-item (calibration) path: video/action are LISTS of per-item tensors
        # [calib_seg... | history | current+future]. Handled separately so the single-item
        # path below is byte-unchanged for all existing datasets/checkpoints.
        if isinstance(data_dict.get("video"), list):
            return self._call_multiitem(data_dict, resolution, mode)
        ...
        num_condition_latent_frames = 1 + (max(1, self.num_history_frames) - 1) // self.video_temporal_downsample
        sequence_plan = build_sequence_plan_from_mode(
            mode=mode,
            ...
            num_condition_latent_frames=num_condition_latent_frames,
        )
    ...
    def _call_multiitem(self, data_dict: dict, resolution, mode) -> dict:
        """Multi-item (calibration) transform. ``video``/``action`` are LISTS of per-item tensors
        ``[calib_seg... | history | current+future]``. Resize each vision item, pad each action item,
        and build ONE ``SequencePlan`` from the last (current+future) item. The packer marks all
        vision items except the last fully-conditioning and aligns each action block to its vision
        item; the current+future item conditions on just the current frame."""
        video_items = data_dict["video"]
        action_items = data_dict.get("action")

        # 1. Resize each vision item to the tier; keep a per-item image_size list.
        resized, image_sizes = [], []
        for it in video_items:
            tmp = self.video_resize({"video": it}, resolution)
            resized.append(tmp["video"])
            image_sizes.append(tmp["image_size"])
        data_dict["video"] = resized
        data_dict["image_size"] = image_sizes

        # 2. Caption metadata + tokenization use the current+future (last) item as the representative
        #    video (it is the generation target); temporarily expose it (+ its image_size) as singles.
        last = resized[-1]
        data_dict["video"] = last
        data_dict["image_size"] = image_sizes[-1]
        if self.prompt_json_formatter is not None:
            data_dict = self.prompt_json_formatter(data_dict)
        else:
            for aug in (self.viewpoint_augmentor, self.duration_fps_augmentor, self.resolution_info_augmentor):
                if aug is not None:
                    r = aug(data_dict)
                    if r is not None:
                        data_dict = r
            if self.idle_frames_augmentor is not None and _should_append_idle_frame_info(mode):
                r = self.idle_frames_augmentor(data_dict)
                if r is not None:
                    data_dict = r
        if self.text_tokenizer is not None:
            data_dict = self.text_tokenizer(data_dict)
        data_dict["video"] = resized  # restore the multi-item list
        data_dict["image_size"] = image_sizes  # restore per-item image_size list

        # 3. SequencePlan from the current+future item. History is its OWN item, so the cf item
        #    conditions on just the current frame (num_condition_latent_frames=1).
        cf_video_len = last.shape[1]  # [C,T,H,W] -> T
        cf_action_len = (
            action_items[-1].shape[0] if isinstance(action_items, list) and action_items else max(cf_video_len - 1, 0)
        )
        sequence_plan = build_sequence_plan_from_mode(
            mode=mode,
            video_length=cf_video_len,
            action_length=cf_action_len,
            video_temporal_downsample=self.video_temporal_downsample,
            num_history_actions=0,
            num_condition_latent_frames=1,
        )
        sequence_plan.share_vision_temporal_positions = False  # calib segments are distinct time states

        # Feature A (history-video/action masking): the dataset chose, per sample, a recent contiguous run
        # of the target modality's REAL history tokens to move out of conditioning. Copy that override onto
        # the plan so the packer noises + supervises those history tokens. None / absent => no-op. Pop it so
        # the non-tensor dict never reaches collation.
        history_mask = data_dict.pop("history_mask", None)
        if history_mask is not None:
            sequence_plan.history_item_index = int(history_mask.get("item_index", -1))
            sequence_plan.history_noise_vision_frames = list(history_mask.get("vision_frames", []))
            sequence_plan.history_noise_action_steps = list(history_mask.get("action_steps", []))

        data_dict["sequence_plan"] = sequence_plan

        # 4. Pad each action item to max_action_dim; per-item raw_action_dim.
        if sequence_plan.has_action and isinstance(action_items, list):
            data_dict["raw_action_dim"] = (
                [torch.tensor(a.shape[1]) for a in action_items] if self.action_channel_masking else None
            )
            data_dict["action"] = [pad_action_to_max_dim(a, self.max_action_dim) for a in action_items]
        else:
            data_dict["raw_action_dim"] = None
            data_dict["action"] = None
            data_dict["domain_id"] = None
