...
from cosmos_framework.data.vfm.action.transforms import ActionTransformPipeline, augment_action_caption_for_inference
...
@pytest.mark.L0
def test_inference_caption_metadata_matches_training_pipeline() -> None:
    pipeline = ActionTransformPipeline(tokenizer_config=None, max_action_dim=4)
    data_dict = {
        "ai_caption": "Open the drawer.",
    ...
    training_caption = pipeline(data_dict, resolution="256")["ai_caption"]
    inference_caption = augment_action_caption_for_inference(
        "Open the drawer.",
        viewpoint="third_person_view",
        num_frames=17,
        fps=15,
    ...
    assert inference_caption == training_caption
