...
- [Installation](#installation)
  - [Conda + uv (recommended)](#conda--uv-recommended)
...
The environment is built in two pinned layers: **conda** supplies the Python interpreter
(`environment.yml`, exact version) and **uv** supplies every Python package from the committed
`uv.lock` (exact versions, including git commits). conda covers only the interpreter because the
rest of the stack — PyTorch + CUDA, flash-attn, NATTEN, Transformer Engine, Megatron-Core — is
published as wheels on PyPI, `download.pytorch.org` and NVIDIA's package index, and is not
available on conda channels.

[Conda + uv](#conda--uv-recommended) is the recommended path. If you already run inside the
[recommended base image](#recommended-base-image) or a container, use
[Quickstart](#quickstart-from-the-recommended-base-image) instead. For other paths (standalone
venv, custom torch/cuda) see [Advanced](#advanced).

<details open><summary><b>Conda + uv (recommended)</b></summary>

### Conda + uv (recommended)
...
Create the environment (pins the interpreter):
...
Install [uv](https://docs.astral.sh/uv/getting-started/installation/) **outside** the environment:
...
> Do not `pip install uv` into the conda environment. `uv` is itself a pinned project dependency
> (`uv==0.10.8`); the next `uv sync` would replace your copy with one older than the
> `required-version = ">=0.11.3"` this project declares, after which `uv` can no longer parse
> `pyproject.toml`.

Sync the locked dependencies into the conda prefix. `UV_PROJECT_ENVIRONMENT` is what makes uv
install into the active conda environment instead of creating its own `.venv`:
...
`--frozen` replays `uv.lock` without re-resolving, which is what makes the install reproducible;
drop it only when you intend to update the lockfile. The command also installs `cosmos_framework`
itself in editable mode, so no separate `pip install -e .` step is needed.
...
If you started from the [recommended base image](#recommended-base-image) (`nvcr.io/nvidia/pytorch:25.09-py3`), the following commands set up the full environment in one go. Run them **from the root of this repository** (i.e. inside the `SyncWorld/` directory you just cloned):
...
Use these paths only when the recommended base image is not viable for your environment.
...
Package extras (`--extra <name>`, or `--all-extras` for all of them):

- `train`: training infrastructure (FSDP, parallelism, checkpointing, datasets) **and** the
  evaluation dependencies, including LPIPS. This is the extra to use for training + evaluation.
- `guardrail`: content-safety filters for inference
- `serve`: online serving (Ray + Gradio)
- `interactive`: notebook helpers
...
2. Authenticate using **either** mechanism (they are equivalent — pick one, do not set both with different tokens):
   - **`HF_TOKEN` environment variable** — preferred for non-interactive shells. Export it once and any `huggingface_hub` call (CLI or library) picks it up.
   - **`uvx hf auth login`** — preferred for local interactive use. Writes the token to `~/.cache/huggingface/token`, persisted across sessions.
...
This applies to the current session only. To persist, add the line to your `~/.bashrc`.
